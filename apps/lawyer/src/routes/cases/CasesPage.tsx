...
import type { LawyerCasesFilterKey } from './components/CasesList';
import { CasesList } from './components/CasesList';
import { parseDateInputValue, toDateInputValue } from './utils/cases.utils';

const FILTERS: { id: LawyerCasesFilterKey; label: string }[] = [
...
export function CasesPage() {
  ...
  const [filter, setFilter]       = useState<LawyerCasesFilterKey>('pending');
  ...
  function handleFilterChange(key: LawyerCasesFilterKey) {
    setFilter(key);
    setPage(1);
  }
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between mb-5">
          <div className="max-w-xl">
            ...
            <p className="mt-1 text-[13px] leading-relaxed text-gray-600 font-sans">
              Manage assigned cases — accept new requests, transition stages, schedule hearings.
            </p>
          </div>
          <Button
            variant="gold"
          ...
          >
            <Plus size={14} /> Open case for client
          </Button>
        </div>
        ...
        <SurfaceCard radius="xl" elevation="sm" padding="none" className="mb-5 p-3 sm:p-4">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
            ...
            <SegmentedControl
              items={FILTERS}
              active={filter}
              // SegmentedControl emits the item id, which is always a LawyerCasesFilterKey here.
              onChange={(id) => handleFilterChange(id as LawyerCasesFilterKey)}
